...
class UpdateAdminView(ModelFormAdminView):
    ...
    def get_form_datas(self):
        ...
        if self.request_method == 'post':
            if '_addanother' in self.request.POST:
                '''这里调整一下修改页面继续增加下一个按键的功能，
                点击按键后增加新条目，并保留页面数据'''
                '''在这里判断是否点击了该按键，如果点击该按键，删掉params中的instance，这样会新增数据
                而不是修改数据，然后在post_response中修改跳转页面'''
                obj_pop=params.pop('instance')
            params.update(
    ...
    def post_response(self):
        ...
        if "_continue" in request.POST:
            ...
            '''在修改页面点击保存并增加下一条的话，页面跳转为新增数据的修改页面'''
            return self.model_admin_url('change', self.new_obj._get_pk_val())
        else:
            self.message_user(msg, 'success')
            # Figure out where to redirect. If the user has change permission,
